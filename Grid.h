...
public:
	...
	void SetCurrPlayerNum(int n);

	Player* GetPlayerByNum(int n);
	...
	CellPosition GetEndSnake(int ps); 
	bool SnakeCheck(Snake* Pnew);
	bool LadderCheck(Ladder* Pnew); 
	bool IsOverlappingSnake(Snake* PNewgame); 
	bool IsOverlappingLadder(Ladder* PNewgame);
	...
	void ClearGrid();                           // Clears Grid Of Objects For Loading Another Grid
